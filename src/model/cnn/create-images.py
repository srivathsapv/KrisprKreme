...
def create_11x11_images():
    ...
    for row in processed_data:
        ...
        file_name = 'seq_{}.png'.format(row['id'])
        seq = row['sequence']
        feature_img = np.ndarray((11, 11, 3))

        trunc_score = int(row['score']*10)/float(10)
        class_label = int(float('%.1f' % trunc_score) * 10)
        two_class_label = 1 if class_label >= 5 else 0

        full_file_name = 'data/bit-images/11x11/{}'.format(file_name)
        ...
        reshaped_features = np.reshape(feature_list, (11, 11)) * 255
    ...
    train_class_file = open('data/img_11x11_train_classes.txt', 'w')
    test_class_file = open('data/img_11x11_test_classes.txt', 'w')
